...
Object.values(carConnModels).forEach((make) => {
  Object.values(make.models).forEach((model) => {
    makesModels.push({
      make: make.name,
      model: model.name,
    });
...
dotModels.forEach((model) => {
  const names = model.Model_Name.split(/\s*?\/\s*?/);
  names.forEach((name) => {
    makesModels.push({
...
const database = cars
  .map((car) => {
    const possibleModels = sortedModels.filter(
      (m) => m.make.toLowerCase() === car.make.toLowerCase()
    );
    ...
    return {
      ...car,
      model,
      modelVerbose: car.model,
    };
  })
  .filter(Boolean);

console.log(
  "Reduced EPA vehicle list from",
